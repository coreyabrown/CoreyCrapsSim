...
def corey(player, table, unit=5, strat_info=None):
    ...
    if table.point == "On":
        if table.point.number in [6, 8] and not player.has_bet("Odds"):
            player.bet(Odds(2 * unit, player.get_bet("PassLine")))
        if player.num_bet("Come") < 2:
            player.bet(Come(unit))
        if not player.has_bet("Come4") and not player.has_bet("Come9") and not player.has_bet("Come10") and player.num_bet("Come") == 2 and not player.has_bet("Field"):
            player.bet(
                Field(
                    unit,
